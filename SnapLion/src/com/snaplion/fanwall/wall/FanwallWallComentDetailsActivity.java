...
public class FanwallWallComentDetailsActivity extends FanwallSuperActivity 
{
    ...
    private boolean oneTimeNotRefreshFlag=true;
    AQuery aq;
        ...
        { 
        	switch(msg.arg1)
        	{
        		case FanwalLoginManager.FLAG_LIKE_SUCCES:
        		{
        			...
        			Utility.debug("FanwallWallComentDetailsActivity like == index "+likeResultedComment.getIndex()+" is like by me :"+likeResultedComment.getLiked_by_me());
...
//        			editor.putString("ARTIST_POST_DATA", null);
...
//        			editor.commit();
        		...
        		case FanwalLoginManager.FLAG_ERROR:
        		{
        			...
        			{
        				Toast.makeText(FanwallWallComentDetailsActivity.this, result, Toast.LENGTH_SHORT).show();
        			}
        			break;
        		}
        		case PostUpdateLocalService.REQTYPE_UPDATE_COMMENT:
	    		{
	    			if(oneTimeNotRefreshFlag)
	    			{
	    				Utility.debug("COMMENT : recieved message from service.");
		    			FanwallWallComentDetailsActivity.this.resetUI(false);
	    			}
	    			else
	    			{
	    				oneTimeNotRefreshFlag=true;
	    			}
	    			break;
	    		}
        	}
	...
	public void resetUI(boolean showProgress)
    {
    	try
    	{
			...
			{
    			ProgressDialog progress = new ProgressDialog(FanwallWallComentDetailsActivity.this);
	...
	private void initUI()
	{
		...
		oneTimeNotRefreshFlag = i.getBooleanExtra("refreshByThread", true);
	}
	
	private void setupAdaptersListeners()
	{
		listHeaderView = getLayoutInflater().inflate(R.layout.commentlist_headerpost_item, null);
	...
	private class DownloadUserCommentsBGTask extends AsyncTask<Void, Integer, Void> 
	{
		...
		protected void onPostExecute(Void result) 
		{
			...
			if(getIntent().getBooleanExtra("isListScrollToBottom", false))
			{
	...
	class ImageAdapter extends BaseAdapter 
	{
        ...
        public View getView(int position, View convertView, ViewGroup arg2) 
        {
			...
			viewHolder.addComment.setBackgroundResource(R.drawable.selector_small_comment_plus_gray);
        	...
        	if(!FanwalLoginManager.getInstance().isUserAlreadyLoggedIn(FanwallWallComentDetailsActivity.this))
        	{
        		viewHolder.likesParentLayout.setVisibility(RelativeLayout.GONE);
				convertView.setPadding(0, 0, 0, 10);
        	}
			...
			viewHolder.addFavorite.setOnClickListener(new OnClickListener() 
			{
				...
				{
					SnapLionMyAppActivity.KillFlag = false;
					FanwalLoginManager.getInstance().likeComment(FanwallWallComentDetailsActivity.this, comment, catName, 2, 12, CURRENT_TAB_WALL);
				}
			});
			...
			if(comment.getMessage_thumb_url()!=null && comment.getMessage_thumb_url().length()>0)
			{
				...
				if(bitmap!=null)
		    	{
		    		aq.id(viewHolder.commentBodyImage_ImageView).image(bitmap);
		    	}
		    	else
		    	{
		    		aq.id(viewHolder.commentBodyImage_ImageView)
			...
			viewHolder.commentBodyImage_ImageViewParent.setOnClickListener(new OnClickListener() 
			{
				...
				{
					SnapLionMyAppActivity.KillFlag = false;
					FanwalLoginManager.getInstance().viewPicture(FanwallWallComentDetailsActivity.this, comment, "CMNT", catName);
				}
			});
			...
			if(comment.getUser_photo_url()!=null && comment.getUser_photo_url().length()>0)
			{
		    	...
		    	else
		    	{
		    		...
		    		.progress(viewHolder.userImage_ProgressBar)
			...
			viewHolder.addComment.setOnClickListener(new OnClickListener() 
			{
				...
				{
					SnapLionMyAppActivity.KillFlag = false;
					FanwalLoginManager.getInstance().addComment(FanwallWallComentDetailsActivity.this, parentPostId, catName, 2 , 12, CURRENT_TAB_WALL);
					adapter.notifyDataSetChanged();
				}
			});
    		return convertView;
	...
	private void addHeaderPostData(final FanwallComment comment)
	{
		try
		{
        	...
        	viewHolder.likedPersonList_TextView.setTextColor(getResources().getColor(R.color.black));
        	viewHolder.replylike_TextView=(TextView)listHeaderView.findViewById(R.id.replylike_TextView);
        	viewHolder.replylike_TextView.setTextColor(getResources().getColor(R.color.black));
        	viewHolder.replylike_TextView.setText("Reply "+clicked_post.getTotal_comments()+" | "+"Like "+comment.getLikes_total());
        	
        	viewHolder.replylikeParent_RelativeLayout=(RelativeLayout)listHeaderView.findViewById(R.id.replylikeParent_RelativeLayout);
        	viewHolder.replylikeParent_RelativeLayout.setVisibility(TextView.VISIBLE);	
			...
			viewHolder.addFavorite.setOnClickListener(new OnClickListener() 
			{
				...
				{
					SnapLionMyAppActivity.KillFlag = false;
					FanwalLoginManager.getInstance().likeComment(FanwallWallComentDetailsActivity.this, comment, catName, 1, 12, CURRENT_TAB_WALL);
				}
			});
	...
	private class ViewHolder
    {
    	...
    	//RelativeLayout addFavoriteWrapperLayout;
    	//RelativeLayout addCommentWrapperRelativeLayout;
...
//		{
//			try{
//				Utility.killMyApp(getApplicationContext(),FanwallWallComentDetailsActivity.this);
	...
	private void showPreviewAppHomeButton()
	{
		...
		if(appManager.PREVIEWAPP_FLAG)
		{
			homeBtn.setOnClickListener(new OnClickListener() 
			{
				...
				public void onClick(View v) 
				{
					...
					Intent i = new Intent(FanwallWallComentDetailsActivity.this,PreviewSnapLionAppsSelectActvity.class);
    				i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
    				FanwallWallComentDetailsActivity.this.startActivity(i);
					finish();
		...
		else
		{
			homeBtn.setOnClickListener(new OnClickListener() 
			{
				...
				{
					SnapLionMyAppActivity.KillFlag = false;
					FanwalLoginManager.getInstance().openLoginActivity(FanwallWallComentDetailsActivity.this, null, catName, CURRENT_TAB_WALL);
				}
			});
		}
	}
					...
					if(type.equalsIgnoreCase("JSON"))
					{
						try
						{
							...
							if(postJsonObject.has("comment_id"))
							{
						    	...
						    	((TextView)listHeaderView.findViewById(R.id.replylike_TextView)).setText("Reply "+clicked_post.getTotal_comments()+" | "+"Like "+clicked_post.getLikes_total());
					        	adapter.notifyDataSetChanged();
					        	scrollMyListViewToBottom(); 
							}
						}
